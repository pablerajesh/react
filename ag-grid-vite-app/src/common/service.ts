import { ICustomer, ICustomerAutocompleteOption, ICustomerHierarchy } from "./types.def";

const totalCustomers: number = 50,
  slotSize: number = 10;

const isParentCustomer = (id: number) => id - 1 === 0 || (id - 1) % slotSize === 0;
...
const persistedCustomers: ICustomer[] = Array.from({ length: totalCustomers }, (_, i) => {
  const id = ++i,
...
export const getParentCustomersFromBackend = (): Promise<ICustomer[]> => {
  const parentCustomers: ICustomer[] = persistedCustomers.filter(customer => customer.isParent);
  return new Promise<ICustomer[]>(resolve => {
    setTimeout(() => {
...
export const getChildCustomersFromBackend = (parentId: number): Promise<ICustomer[]> => {
  const childCustomers: ICustomer[] = persistedCustomers.filter(customer => customer.parentId === parentId);
  return new Promise<ICustomer[]>(resolve => {
    setTimeout(() => {
...
export const getCustomerAutocompleteOptions = (customers: ICustomer[]): ICustomerAutocompleteOption[] =>
...
export const getCustomerHierarchies = (): Promise<ICustomerHierarchy[]> => {
  ...
  const parents: ICustomer[] = persistedCustomers.filter(customer => customer.isParent);
  ...
  parents.forEach(parent => {
    ...
    const children: ICustomer[] = customers.filter(customer => customer.parentId === parent.id);
