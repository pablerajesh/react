...
import { customerColDefs, customerDefaultCollDef, ICustomer } from "../../common/types.def";

export interface IOrphanCustomersDisplayProps {
  orphanCustomers: ICustomer[] | undefined;
}

export const OrphanCustomersDisplay = ({ orphanCustomers }: IOrphanCustomersDisplayProps) => {
  const defaultColDef = useMemo<ColDef<ICustomer>>(() => customerDefaultCollDef, []);
  const [columnDefs] = useState<ColDef<ICustomer>[]>(customerColDefs);
  const [rowData, setRowData] = useState<ICustomer[] | undefined>(orphanCustomers);
